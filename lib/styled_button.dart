...
import 'package:styled_button/image_assets.dart';
...
class StyledButton extends StatelessWidget {
  ...
  final AuthButtonProvider
      provider; // The provider (e.g., Google, Facebook, etc.)
  final Color? buttonColor,
      borderColor,
      textColor; // Optional colors for button, border, and text
  final String? text,
      customLogoPath; // Optional text for the button and logo image path
  final bool disableIcon, isBorder; // Flag to disable the icon
  ...
  final double? buttonHeight,
      ...
      borderWidth,
      iconSize,
      textSize; // Optional styling properties
  ...
  const StyledButton({
    ...
    this.disableIcon =
        false, // Default value for disabling icon (false = enabled)
    this.disableText =
        false, // Default value for disabling text (false = enabled)
    ...
    this.gradient,
    this.isBorder = true, // Optional gradient for button background
  });
  ...
  Widget build(BuildContext context) {
    ...
    if (customLogoPath == null) {
      ...
      switch (provider) {
        case AuthButtonProvider.google:
          logoPath = ImageAssets.google; // Google logo
          break;
        case AuthButtonProvider.facebook:
          logoPath = ImageAssets.facebook; //Facebook logo
          break;
        case AuthButtonProvider.twitter:
          logoPath = ImageAssets.twitter; // Twitter logo
          break;
        case AuthButtonProvider.apple:
          logoPath = ImageAssets.apple; // Apple logo
          break;
        case AuthButtonProvider.github:
          logoPath = ImageAssets.google; // GitHub logo
          break;
        case AuthButtonProvider.linkedin:
          logoPath = ImageAssets.linkedin; //  LinkedIn logo
          break;
        case AuthButtonProvider.microsoft:
          logoPath = ImageAssets.microsoft; // Microsoft logo
          break;
        case AuthButtonProvider.instagram:
          logoPath = ImageAssets.instagram; // Instagram logo
          break;
        case AuthButtonProvider.discord:
          logoPath = ImageAssets.discord; //Discord logo
          break;
        case AuthButtonProvider.phone:
          logoPath = ImageAssets.phone; // Phone logo
          break;
      }
    }
        ...
        disableIcon // If the icon is disabled, show an empty space
            ...
            : customLogoPath != null // If custom logo path is provided, use it
                ? Image.asset(
                    customLogoPath!,
                    height:
                        (iconSize != null && iconSize! <= 34) ? iconSize : 20.0,
                    width:
                        (iconSize != null && iconSize! <= 34) ? iconSize : 20.0,
                  )
                : Image.asset(
                    ...
                    package:
                        'styled_button', // Use the logo image from the package assets
                  );

    // Setting spacing between icon and text based on whether they are enabled
    final spacing = (!disableIcon &&
            !disableText) // If both icon and text are enabled, add spacing between them
        ? const SizedBox(width: 10.0)
        : const SizedBox
            .shrink(); // No spacing if either icon or text is disabled
    ...
    final textWidget =
        disableText // If the text is disabled, show an empty space
            ? const SizedBox.shrink()
            : Flexible(
                // Wrap text in a Flexible widget to ensure it fits within available space
                child: Text(
                  text ??
                      '', // Display the text, defaulting to empty if not provided
                  textAlign: TextAlign.center, // Center-align the text
                  overflow:
                      TextOverflow.ellipsis, // Show ellipsis if text overflows
                  style: textStyle ?? // Use custom text style or default style
                      TextStyle(
                        color: textColor ??
                            Colors.black, // Text color, default is black
                        fontSize:
                            textSize ?? 18.0, // Text size, default is 18.0
                        fontWeight: fontWeight ??
                            FontWeight.bold, // Font weight, default is bold
                      ),
                ),
              );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        width: buttonWidth ??
            double.infinity, // Set button width, default is full width
        padding: const EdgeInsets.symmetric(
            horizontal: 10), // Horizontal padding inside button
        decoration: BoxDecoration(
          color: gradient == null
              ? (buttonColor ?? Colors.white)
              : null, // Set solid color or gradient as background
          gradient: gradient, // Apply gradient if provided
          borderRadius:
              BorderRadius.circular(borderRadius ?? 10.0), // Set border radius
          border: isBorder == true
              ? Border.all(
                  color: borderColor ??
                      Colors.grey, // Border color, default is grey
        ...
        child: Row(
          mainAxisSize:
              MainAxisSize.min, // Make row size fit the content (icon + text)
          mainAxisAlignment:
              MainAxisAlignment.center, // Center the content horizontally
          children: iconSide ==
                  IconSide
                      .left // Check if the icon should be on the left or right side
              ? [
                  imageWidget,
                  spacing,
                  textWidget
              ...
              : [
                  ...
                  spacing,
                  imageWidget
                ], // Place text on left, icon on right
        ),
      ),
    );
  }
